...
# required_args.add_argument('-config_profile', required=True, nargs=1, type=str, help='filesetid ex: SPNWTCN2DA')
# required_args.add_argument('-filesetid', required=True, nargs=1, type=str, help='filesetid ex: SPNWTCN2DA')
# args = parser.parse_args()
# input_filesetid=args.filesetid[0]
...
class AudioHLS:
    ...
    def processCommandLine(self):
        if len(sys.argv) < 3:
            print(
                "ERROR: Enter config_profile and any number of bibleids or bibleid/filesetids to process on command line.")
            sys.exit()
        ...
        for arg in arguments:
            ...
            if len(parts) > 1:
                ...
                self.processmultiple_FilesetIds(arg)
        self.close()
    ...
    def validateArguments(self, arguments):
        ...
        for arg in arguments:
            parts = arg.split("/")
            if len(parts) == 1:
                bibleId = self.adapter.get_Bibleid(arg)
                print(bibleId)
                if self.adapter.checkBibleId(bibleId) < 1:
                    errorCount += 1
                    print("ERROR: bibleid: %s does not exist." % (arg))
                elif self.adapter.checkBibleTimestamps(bibleId) < 1:
                    errorCount += 1
    ...
    def processmultiple_FilesetIds(self, filesetid):
        # Get bibleId
        ...
        bibleId = self.adapter.get_Bibleid(filesetid)
        for filesetId in [filesetid]:
            ...
            if bitrate != "64":
                ...
                for row in resultSet:
                    ...
                    if (maxDuration - minDuration) > self.audio_hls_duration_limit:
                        ...
                        print("WARN: %s/%s/%s was dropped, because of duration difference %d to %d" % (
                            bibleId, bitrateFilesetId, fileName, minDuration, maxDuration))
    ...
    def getBitrate(self, file):
        ...
        for line in pipe.stdout:
            result = self.bitrateRegex.search(str(line))
        if result != None:
...
class AudioHLSAdapter:
    ...
    def get_Bibleid(self, filesetid):
        sql = "SELECT hash_id FROM bible_filesets WHERE id=%s"
        hash_id = self.selectScalar(sql, (filesetid,))
        sql = "SELECT bible_id FROM bible_fileset_connections WHERE hash_id=%s"
        return self.selectScalar(sql, (hash_id,))
