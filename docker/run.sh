...
transcoder.ecs_region = us-west-2
transcoder.ecs_disabled = ${TRANSCODER_ECS_DISABLED}
transcoder.ecs_cluster_name = ${TRANSCODER_ECS_CLUSTER_NAME}
transcoder.ecs_container_name = ${TRANSCODER_ECS_CONTAINER_NAME}
transcoder.ecs_placement_security_group = ${TRANSCODER_ECS_PLACEMENT_SECURITY_GROUP}
transcoder.ecs_placement_subnet = ${TRANSCODER_ECS_PLACEMENT_SUBNET}
transcoder.ecs_task_definition = ${TRANSCODER_ECS_TASK_DEFINITION}
...
audio.transcoder.url = ${AUDIO_TRANSCODER_BASE_URL}
...
if [ -n "${LPTS_UPLOAD:-}" ]; then
  ...
  echo "run.sh second branch. DATA_MODEL_MIGRATION_STAGE: $DATA_MODEL_MIGRATION_STAGE "
  if [ "$DATA_MODEL_MIGRATION_STAGE" == "B" ]; then
