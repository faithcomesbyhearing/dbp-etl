	...
	def bibleIds(self, filesetId, typeCode):
		...
		if id1 != None:
			if id1 in self.bibleIdList:
				...
				print("WARNING: filesetId %s not added, because bibleId %s not in LPTS(1)" % (filesetId, id1))
		id2 = lookup[filesetId[:10]].DBP_Equivalent2()
		if id2 != None:
			if id2 in self.bibleIdList:
				results.append(id2)
			else:
				print("WARNING: filesetId %s not added, because bibleId %s not in LPTS(2)" % (filesetId, id2))
		return results
